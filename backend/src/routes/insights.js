...
const { validate, insightCreateSchema } = require('../middleware/validate');
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const key = cache.KEY.insights(req.userId);
    ...
    const payload = insights.map(toClientInsight);
    await cache.set(key, payload, cache.TTL.INSIGHTS);
    res.json(payload);
  })
);
...
router.post(
  '/',
  validate(insightCreateSchema),
  asyncHandler(async (req, res) => {
    const {
      awareness_level, reasoning_summary, symptom_impacts, correlations,
      red_flags, doctor_nudge, doctor_nudge_reason, weekly_trend_summary,
      log_count_analyzed, date_range_start, date_range_end,
    } = req.body;
