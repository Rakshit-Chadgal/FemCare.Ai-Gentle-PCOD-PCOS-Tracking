...
export default function Layout() {
  ...
  useEffect(() => {
    async function checkOnboarding() {
      try {
        const fetchedProfile = await userProfileService.get();
        const isLocalOnboarded = localStorage.getItem('femcare_onboarded') === 'true';

        if ((!fetchedProfile || !fetchedProfile.onboarding_completed) && !isLocalOnboarded) {
          navigate('/onboarding', { replace: true });
          return;
        }

        setProfile(fetchedProfile && Object.keys(fetchedProfile).length > 0
          ? fetchedProfile
          : { onboarding_completed: true, display_name: 'Friend' }
        );
      } catch (e) {
        console.error('Failed to fetch profile:', e);
        // Fallback so user is not stuck on a loading screen
        setProfile({ onboarding_completed: true, display_name: 'Friend' });
      } finally {
        setLoading(false);
      }
    }
